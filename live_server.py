...
class LiveServer:
    ...
    def __init__(self, measurement_script):
        ...
        self.__measurement_thread = MeasurementThread(self.__socketio.emit, self.__measurement_script)
        ...
        def handle_get_device_info():
            emit("update_device_info", self.__measurement_script.get_device_info())

        @self.__socketio.on("get_datasets")
        def handle_get_datasets():
            self.__dataset_manager.scan_datasets()
            dataset_list = [{"name": dataset.name, "is_compatible": dataset.is_device_compatible(self.__measurement_script.get_device_info())} for dataset in self.__dataset_manager.datasets]
        ...
        def handle_select_dataset(selected):
            ...
            if (self.__selected_dataset != None):
                self.__selected_dataset.get_write_access(self.__measurement_script.get_device_info())
        ...
        def handle_add_dataset(name):
            self.__selected_dataset = self.__dataset_manager.create_dataset(name, self.__measurement_script.get_device_info())
            dataset_list = [{"name": dataset.name, "is_compatible": dataset.is_device_compatible(self.__measurement_script.get_device_info())} for dataset in self.__dataset_manager.datasets]
            dataset_name = self.__selected_dataset.name if self.__selected_dataset != None else None
